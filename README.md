...
- **cron 故意设在 `:23` 而不是 `:00`**：GitHub 的定时任务和全平台所有 push/PR
  共用同一个队列，整点是最堵的时候——排队的任务不只是会延迟，**经常直接被丢弃**。
  实测这个 repo 用 `0 * * * *` 连续 4 个整点一次都没触发，改成 `23 * * * *` 后正常。
  即使这样也还是会有几分钟浮动，属正常。
- **定时任务不保证准时**：GitHub 官方就说了 schedule 会在高负载时延迟。
  如果哪天你需要"分钟级必达"（比如抢限量），别指望 GitHub cron，
  得用自己的服务器跑 cron + 调 `workflow_dispatch` API。
